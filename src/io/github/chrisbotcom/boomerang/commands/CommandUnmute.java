...
public class CommandUnmute implements CommandExecutor {
    ...
    public CommandUnmute(Boomerang plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (args.length > 1) {
            sender.sendMessage(ChatColor.RED + "/unmute accepts one optional argument.");
            return false;
        }
        ...
        if (args.length == 0) {
            ...
            if (player == null) {
                ...
                plugin.getSettings().getMuteList().remove(player);
